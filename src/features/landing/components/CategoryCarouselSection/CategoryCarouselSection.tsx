...
export const CategoryCarouselSection = ({ visibleRange, currentIndex, playingCategory, onCategoryClick, onPrev, onNext, onGoTo, onPlayCategory, onPauseCategory }: CategoryCarouselSectionProps) => {
	...
	return (
		<BaseCarouselSection
			...
			onItemClick={(index: number) => onCategoryClick(index)}
			onPrev={onPrev}
			onNext={onNext}
			onGoTo={onGoTo}
			onPlay={(id: string | ThemeCategory) => onPlayCategory(id as ThemeCategory)}
			onPause={onPauseCategory}
			getPlayingId={(item: { category: ThemeCategory }) => item.category}
			titleTag="h2"
